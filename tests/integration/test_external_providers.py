...
async def test_antigravity_ask_returns_non_empty_response() -> None:
    _require_cli("agy")
    provider = AntigravityProvider(timeout_seconds=60.0)
    try:
        response = await provider.ask("Responde Hola", _context(), "ignored")
    finally:
        await provider.close()
...
async def test_antigravity_timeout_raises_provider_timeout_error() -> None:
    _require_cli("agy")
    provider = AntigravityProvider(timeout_seconds=0.001)
    try:
        with pytest.raises(ProviderTimeoutError):
            await provider.ask("Responde Hola", _context(), "ignored")
    finally:
        await provider.close()
...
async def test_antigravity_raises_unavailable_when_binary_missing() -> None:
    ...
    with pytest.raises(ProviderUnavailableError):
        await provider.ask("hello", context=_context(), model="ignored")
...
async def test_antigravity_stream_raises_not_implemented() -> None:
    ...
    with pytest.raises(NotImplementedError):
        await provider.stream("hello", context=_context(), model="ignored")
...
async def test_opencode_stream_raises_not_implemented() -> None:
    provider = OpenCodeProvider()
    with pytest.raises(NotImplementedError):
        await provider.stream("hello", context=_context(), model="openai/gpt-4o")
